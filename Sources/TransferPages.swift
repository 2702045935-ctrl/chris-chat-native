        ...
        ScrollView {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 16) {
                    ...
                    HStack {
                        Text("收款人").foregroundColor(.secondary)
                        Spacer()
                        Text(receiverName)
                    }
                    HStack {
                        Text("收款账号").foregroundColor(.secondary)
                        Spacer()
                    ...
                    if !remark.isEmpty {
                        HStack {
                            Text("备注").foregroundColor(.secondary)
                            Spacer()
                            Text(remark)
                        }
                    }
...
struct TransferPasswordView: View {
    ...
    var body: some View {
        ...
        .alert("忘记支付密码", isPresented: $showForgot) {
            ...
            Text("支付密码在「我 → 设置 → 安全中心 → 支付密码」里可以重设；重设后回来继续转账。")
        }
